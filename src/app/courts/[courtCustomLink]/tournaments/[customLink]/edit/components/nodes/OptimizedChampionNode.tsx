...
const OptimizedChampionNode: React.FC<{ data: ChampionNodeData; id: string }> = React.memo(({ data, id }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`${STYLES.championNode} w-full h-full flex items-center justify-center relative px-2 transition-colors duration-200`}
      >
        <div className="absolute inset-0 bg-yellow-200 dark:bg-yellow-700 rounded animate-pulse opacity-30"></div>
        <span className={`${STYLES.championText} flex-1 relative`}>{champion.name}</span>
