...
const CushionCalculator: React.FC = () => {
  ...
  const { width, height } = calculatorDimensions;
  // 參數和常數
  ...
  if (isMobileVertical) return <label>請將手機旋轉成橫向以顯示撞球桌</label>;

  return (
    <section className="flex flex-col items-center">
      <section className="flex flex-col items-center mt-[20px] mb-[30px] gap-4">
        <div className="flex flex-row items-center gap-4">
          <h1 className="text-2xl font-bold">{PageType.CUSHION}</h1>
      ...
      <Popup title={`${PageType.CUSHION}說明`} display={showTips} onClose={() => setShowTips(false)}>
        <Tips />
      </Popup>
    </section>
  );
